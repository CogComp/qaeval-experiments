...
def main(args):
    ...
    with open(args.output_csv, 'w') as out:
        with open(args.output_mapping_csv, 'w') as out_mapping:
            writer = csv.writer(out)
            writer_mapping = csv.writer(out_mapping)
            writer.writerow([
                'instance_id',
                'question_id',
                'question',
                        ...
                        for reference in instance['references']:
                            for question_dict in reference['questions']:
                                ...
                                if is_answerable:
                                    ...
                                    # For each instance and question, it's probably safe to skip any pairs which are the same
                                    key = (instance_id, question_id, answer.lower(), prediction.lower())
                                    if key in pair_seen:
                                        num_skipped += 1
                                        continue
                                    pair_seen.add(key)
...
if __name__ == '__main__':
    ...
    argp.add_argument('output_mapping_csv')
