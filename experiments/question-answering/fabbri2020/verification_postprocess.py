...
def load_annotations(file_path: str, expert_answers, model_answers):
    # Load the mapping from (instance_id, question_id, prediction) -> (instance_id, summarizer_id, prompt_id, prediction)
    # This is a hack because we only annotated (instance_id, question_id, prediction) but it turns out that some
    # prompts map to multiple questions, so we need to know what the updated key is
    mapping = defaultdict(list)
    for answer_set in [expert_answers, model_answers]:
        for instance in answer_set:
            ...
            for reference in instance['references']:
                for question in reference['questions']:
                    ...
                    mapping[(instance_id, question_id, prediction)].append((instance_id, summarizer_id, prompt_id, prediction))
    ...
    with open(file_path, 'r') as f:
        ...
        for i, row in enumerate(reader):
            if i == 0:
                ...
                instance_id, question_id, question, answer, prediction, is_correct, is_intended_answer = row
                is_correct = is_correct.lower() == 'true'
                is_intended_answer = is_intended_answer.lower() == 'true'
                if prediction is not None:
                    prediction = prediction.lower()
                for o_instance_id, o_summarizer_id, o_prompt_id, o_prediction in mapping[(instance_id, question_id, prediction)]:
                    annotations_dict[(o_instance_id, o_summarizer_id, o_prompt_id, o_prediction)] = (is_correct, is_intended_answer)
...
def main(args):
    ...
    annotations_dict = load_annotations(args.annotations_csv, expert_answers, model_answers)
